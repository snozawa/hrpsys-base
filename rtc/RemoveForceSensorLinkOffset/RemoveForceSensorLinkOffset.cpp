...
RTC::ReturnCode_t RemoveForceSensorLinkOffset::onInitialize()
{
  ...
  // serialized data
  m_serializedStateData.data.length(6*m_forceIn.size());
  return RTC::RTC_OK;
}
      ...
      if ( m_force[i].data.length()==6 ) {
        ...
        if ( sensor ) {
          ...
          for (size_t j = 0; j < 3; j++) {
            ...
            m_serializedStateData.data[i*6+j] = m_force[i].data[j];
            m_serializedStateData.data[i*6+3+j] = m_force[i].data[3+j];
          }
          if ( DEBUGP ) {
